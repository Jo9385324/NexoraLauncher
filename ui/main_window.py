...
class MainWindow(ctk.CTk):
    ...
    def _create_sidebar(self) -> None:
        ...
        nav_items = [
            ("home", "🏠  Главная", 1),
            ("instances", "📁  Инстансы", 2),
            ("versions", "📦  Версии", 3),
            ("mods", "🧩  Моды", 4),
            ("shaders", "🌈  Шейдеры", 5),
            ("servers", "🖥️  Серверы", 6),
            ("ai", "🤖  AI Помощник", 7),
            ("settings", "⚙️  Настройки", 8),
        ]
        ...
        for idx, (page_id, label, grid_row) in enumerate(nav_items, start=1):
            ...
            btn.grid(row=grid_row, column=0, sticky="ew", padx=15, pady=3)
            self.nav_buttons[page_id] = btn

        # Версия внизу с улучшенным стилем
        version_frame = ctk.CTkFrame(self.sidebar, fg_color="transparent")
        version_frame.grid(row=9, column=0, padx=20, pady=(0, 5))
        ...
        author_label.grid(row=10, column=0, pady=(0, 15))
